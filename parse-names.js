...
// ported to JavaScript by Mark Pemburn (pemburn.com)
...
function NameParse() {
    ...
    NameParse.prototype.parse = function (fullastName) {
        ...
        if ((end - start) > 1) {
            ...
            for (i; i<end; i++) {
                lastName += " " + me.fix_case(nameParts[i]);
            }
        } else {
    ...
    this.is_compound_lastName = function (word) {
        ...
        words = ['vere','von','van','de','del','della','di','da','pietro','vanden','du','st.','st','la','lo','ter'];
